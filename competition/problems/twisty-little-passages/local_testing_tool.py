...
def RunCases():
  ...
  for case_number in range(NUM_CASES):
    ...
    #print(f"[Debug] Case #{case_number}: =========", file=sys.stderr);
    #for i in range (N):
    #    print("[Debug] node[{}]: {}".format(i, adj[i]), file=sys.stderr);
    ...
    for move_number in range(K+1):
      ...
      if complement:
        ...
        '''
        if move_number < 10:
              print("[Debug] Case #{}: cur move {} {} {}".format( \
                    case_number, move_number, \
                    where+1, N-1-len(adj[where])), \
                    file=sys.stderr)
        '''
      else:
      ...
      if move[0] == "E":
        ...
        #print(f"[Debug] Case #{case_number}: cur move {move_number}", file=sys.stderr);
